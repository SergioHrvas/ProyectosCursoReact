...
function useCart() {
    ...
    */
    ...
    function addToCart(element: Instrument) {
        ...
        if (itemExists === -1) { //No existe en el carrito
            const newItem: CartItem = {...element, count: 1 }
    ...
    function removeFromCart(id: Instrument['id']) {
        //setCart(cart.filter(element => element.id !== id))

        setCart(prevCart => prevCart.filter(element => element.id !== id))
    }

    function changeCount(id: Instrument['id'], inc: boolean) {
